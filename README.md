# yandex2mqtt
TODO: переписать/ актуализировать README.
В отличие от оригинальной, заброшенной автором, версии тут полностью поддерживаются devices.properties и relative значения.
так же, 
1) эта версия не требует передачи custom_data,  собирая mqtt_topic динамически.
2) подразумевает, что SSL терминируется где-то в другом месте.
3) берет параметры для подключения к mqtt брокеру не только из конфига, но и из переменных окружения MQTT_USER/MQTT_PASSWORD, что удобнее.
